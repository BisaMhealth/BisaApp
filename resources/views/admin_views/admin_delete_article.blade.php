        ...
        <div class="row">
            <div class="col-md-12">
               @if($article_to_delete)
                 <h5 style="color: red;">Are you sure you want to delete this article?(This action cannot be reverted.)</h5><br>
		    <form method="post" action="{{route('confirmdeleteArticle',$article_to_delete->article_id)}}" enctype="multipart/form-data">
			@csrf
                        <input value="{{$article_to_delete->article_id}}" name="delete_id" type="hidden" class="edit-article-id">
            
                        <div class="row">
                            <div class="col-6">
                                <label><h4>Title</h4></label>
                                <input type="text" class="form-control form-control-sm edit-article-title" value="{{$article_to_delete->article_title}}" readonly>
                            </div>

                            <div class="col-6">
                                <label><h4>Category</h4></label>
                            <select class="form-control form-control-sm publish-article-select edit-article-cateogory" readonly>
                                <option selected>{{\App\Models\ArticleCategory::select('category_name')->where('category_id',$article_to_delete->article_cat_id)->pluck('category_name')->first()}}</option>
                            </select>
                            </div>
                        </div><br>
                            ...
                            <div class="col-md-8">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control tinymce-editor">
                                            {{$article_to_delete->article_content}}
                                    </textarea>
                                </div>
                            </div>

        			    <div class="col-md-4">
        			    <label><h4>Thumbnail</h4></label>
                            <img src="{{$article_to_delete->article_thumbnail}}" class="img-fluid" style="border-radius: 5px;">
        			    </div>
			    
                        </div>
                        
                        <button type="submit" class="btn btn-sm btn-danger edit-btn">Yes, Delete</button>
                        <a href="/admin/articles" class="btn btn-sm btn-info">No, Cancel</a>
                           
                        
		    </form>
		    @else
    		    <h4>Hmmm, No action was selected</h4>			    
		      <a href="/admin/articles" class="btn btn-sm btn-info">Okay, take me home</a>	
               @endif
            </div>
        </div>
